...
import numpy as np
import glob
import analyze.contacts_per_frame as contacts_analysis
from tqdm import tqdm
import MDAnalysis as mda
from tempfile import NamedTemporaryFile
import analyze.lie_2parts as lie
def find_substrings(sequence, substrings):
    """Find which substrings are present in the given sequence."""
    return [s for s in substrings if s in sequence[90:120]]
...
def parse_cdrs_dir(dcd_directory, pdb_directory, inp_data_file):
    ...
    for pdbit, dcdit, logit in tqdm(zip(all_pdbs, all_dcds, all_logs)):
        try:
            ...
            dict_cdr['location'].append(dcdit)
...
print(l_en_init)
# Example usage
dcd_directory = "trials/T3_ant3HFM_body4NCO_simulations_afterdpo/"
pdb_directory = 'sim_pdbs'

dict_cdr_t2_afterdpo = parse_cdrs_dir(dcd_directory, 
               pdb_directory, 
               't2_all_cdrs_ant3hfm_body4nco_afterdpo.csv'
               )

df_cdr_t2_afterdpo = pd.DataFrame(dict_cdr_t2_afterdpo)
df_cdr_t2_afterdpo.to_csv('t2_afterdpo_gen_cdrs_mden.csv', index=False)
